...
# Navigation Data
ship_trail = []  # List of (x, y) tuples
last_trail_record_time = 0
eta_text = "--:--"
xte_text = "XTE: 0.0"

# Anchor Watch
anchor_watch_active = False
anchor_pos = {'x': 0, 'y': 0}
anchor_radius = 50.0
anchor_alarm = False
...
def update_waypoint_list():
    ...
    # Update tab label to show counts
    if buoys:
        tab_control.tab(waypoint_tab, text=f"Route ({len(waypoints)}) | Buoys ({len(buoys)})")
    else:
        tab_control.tab(waypoint_tab, text=f"Route ({len(waypoints)})")
...
def load_waypoints():
    ...
    try:
        ...
        # Automatically start autopilot when new waypoints are loaded
        if waypoints and not autopilot_active:
            toggle_autopilot()
...
def update_display():
    ...
    if x is not None:
        try:
            ...
            if autopilot_active and target_waypoint_index is not None and 0 <= target_waypoint_index < len(waypoints):
                target_wp = waypoints[target_waypoint_index]
                dist = math.hypot(target_wp['x'] - x, target_wp['y'] - y)
                
                # ETA
                if ship_speed > 0.1:
                    seconds_left = dist / ship_speed
                ...
                # Need previous waypoint to define the track line
                prev_wp = None
                if target_waypoint_index > 0:
                    prev_wp = waypoints[target_waypoint_index - 1]
                elif autopilot_initial_pos:
                    prev_wp = {'x': autopilot_initial_pos[0], 'y': autopilot_initial_pos[1]}
                
                if prev_wp:
                    # Line from prev_wp to target_wp
                    x1, y1 = prev_wp['x'], prev_wp['y']
                    x2, y2 = target_wp['x'], target_wp['y']
                    ...
                    if denominator > 1e-6:
                        ...
                        # Convention: Positive means ship is to the Right of track (need to steer Left)
                        # Negative means ship is to the Left of track (need to steer Right)
                        side = "R" if xte > 0 else "L"
                        xte_text = f"XTE: {abs(xte):.1f} {side}"
                    else:
                        xte_text = "XTE: 0.0"
                else:
                    xte_text = "XTE: --"
            ...
            xte_label.config(text=xte_text)
            
            # Drift Calculation
            drift = (calc_heading - current_heading + 180) % 360 - 180
            drift_label.config(text=f"Drift: {drift:+.1f}°")

            # --- Anchor Watch Check ---
            if anchor_watch_active:
                dist_to_anchor = math.hypot(x - anchor_pos['x'], y - anchor_pos['y'])
                if dist_to_anchor > anchor_radius:
                    if not anchor_alarm:
                        anchor_alarm = True
                        status_label.config(text="⚠️ ANCHOR DRAG ALARM! ⚠️", bootstyle="danger")
                        # Flash effect could be added here
                else:
                    if anchor_alarm:
                        anchor_alarm = False
                        status_label.config(text="Anchor watch active - Safe", bootstyle="success")

            # --- Ship Trail Recording ---
            global last_trail_record_time
            if current_time - last_trail_record_time > 1.0: # Record every 1s
                if not ship_trail or math.hypot(ship_trail[-1][0] - x, ship_trail[-1][1] - y) > 5.0: # Only if moved > 5 units
                    ship_trail.append((x, y))
                    if len(ship_trail) > 1000: # Limit trail length
                        ship_trail.pop(0)
                    last_trail_record_time = current_time
...
def toggle_heading_lock():
    ...
    if heading_locked:
        ...
        locked_heading = current_heading
        heading_lock_label.config(text=f"Locked at {locked_heading:.1f}°", bootstyle="danger")
        status_label.config(text=f"Heading locked at {locked_heading:.1f}°")
    else:
        heading_lock_label.config(text="Unlocked", bootstyle="success")
        status_label.config(text="Heading unlocked")

def toggle_position_lock_hotkey():
    """Toggle both X and Y position locks via hotkey"""
    try:
        # If either is unlocked, lock both. If both are locked, unlock both.
        should_lock = not (x_lock_var.get() and y_lock_var.get())
        
        x_lock_var.set(should_lock)
        toggle_x_lock()
        
        y_lock_var.set(should_lock)
        toggle_y_lock()
        
        state = "Locked" if should_lock else "Unlocked"
        status_label.config(text=f"Position (X & Y) {state}")
    except Exception as e:
        print(f"Position lock toggle error: {e}")
...
def toggle_x_lock():
    ...
    if x_locked:
        ...
        if x is not None:
            locked_x = x
            x_lock_label.config(text=f"Locked", bootstyle="danger")
        ...
        x_lock_label.config(text="Unlocked", bootstyle="success")
...
def toggle_y_lock():
    ...
    if y_locked:
        ...
        if y is not None:
            locked_y = y
            y_lock_label.config(text=f"Locked", bootstyle="danger")
        ...
        y_lock_label.config(text="Unlocked", bootstyle="success")
...
def draw_compass():
    ...
    for angle in range(0, 360, 30):
        ...
        x2 = center_x + radius * 1.0 * math.cos(rad)
        y2 = center_y + radius * 1.0 * math.sin(rad)
        canvas.create_line(x1, y1, x2, y2, width=1, fill="#666666")
        
        # Add degree labels
        if angle % 90 != 0: # Skip cardinals
            label_x = center_x + radius * 0.75 * math.cos(rad)
            label_y = center_y + radius * 0.75 * math.sin(rad)
            canvas.create_text(label_x, label_y, text=f"{angle}", font=("Segoe UI", 8), fill="#888888")
    ...
    for angle, label, color in directions:
        rad = math.radians(angle - 90)
        x = center_x + radius * 1.15 * math.cos(rad)
        y = center_y + radius * 1.15 * math.sin(rad)
        
        font_weight = "bold" if label == "N" else "normal"
        canvas.create_text(x, y, text=label, font=("Segoe UI", 14, font_weight), fill=color)
    ...
    arrow_color = "#ff4444" if heading_locked else "#00aaff"
    
    # Draw needle (triangle shape)
    tip_x = center_x + pointer_length * math.cos(rad)
    tip_y = center_y + pointer_length * math.sin(rad)
    
    # Simple needle line for now
    canvas.create_line(center_x, center_y, tip_x, tip_y,
                      fill=arrow_color, width=3, arrow=tk.LAST, arrowshape=(16, 20, 6))

    # Draw target heading bug if locked
    if heading_locked:
        target_rad = math.radians(locked_heading - 90)
        bug_x = center_x + radius * math.cos(target_rad)
        bug_y = center_y + radius * math.sin(target_rad)
        
        # Draw a little bug marker on the ring
        bug_size = 6
        canvas.create_oval(bug_x - bug_size, bug_y - bug_size,
                          bug_x + bug_size, bug_y + bug_size,
                          fill="#ffaa00", outline="#ffaa00")
    ...
    try:
        ...
        if wp_index is not None:
            ...
            if abs(dx) + abs(dy) > 1e-6:
                ...
                bearing = (90 - bearing) % 360 # Convert to compass heading
                
                bearing_rad = math.radians(bearing - 90)
                bx = center_x + radius * 0.6 * math.cos(bearing_rad)
                by = center_y + radius * 0.6 * math.sin(bearing_rad)
                
                # Draw bearing pointer
                canvas.create_line(center_x, center_y, bx, by,
                                  fill="#00ff00", width=2, arrow=tk.LAST, arrowshape=(10, 12, 4))
                
                # Bearing label inside compass
                canvas.create_text(center_x, center_y + radius - 15,
                                   text=f"BRG {bearing:.0f}°",
                                   font=("Segoe UI", 9, "bold"), fill="#00ff00")
    except Exception:
        pass
...
def fly_to_pos(x, y):
    """Instant autopilot to specific coordinates"""
    global waypoints, autopilot_active, target_waypoint_index
    # Add waypoint at top
    wp = {
        'time': 0,
        'x': x,
    ...
    globals()['map_static_dirty'] = True
    draw_map()
    status_label.config(text=f"Flying to X:{x:.1f} Y:{y:.1f}")
    log_event(f"Fly To initiated: {x:.1f}, {y:.1f}")

def set_anchor_at(x, y):
    """Set anchor watch at specific coordinates"""
    global anchor_watch_active, anchor_pos, anchor_alarm
    anchor_pos = {'x': x, 'y': y}
    anchor_watch_active = True
    anchor_alarm = False
    anchor_btn.config(text="⚓ Drop Anchor", bootstyle="danger")
    status_label.config(text=f"Anchor watch set at X:{x:.1f} Y:{y:.1f}")
    log_event(f"Anchor set at: {x:.1f}, {y:.1f}")
    draw_map()
...
def open_map_window():
    ...
    ttk.Checkbutton(toolbar, text="Show Buoys", variable=buoys_var, command=toggle_buoys, bootstyle="round-toggle").pack(side=tk.LEFT, padx=4)

    # Radar visibility toggle
    radar_var = tk.BooleanVar(value=True)
    def toggle_radar():
        global radar_visible
        radar_visible = radar_var.get()
        draw_map()
    
    ttk.Checkbutton(toolbar, text="Radar", variable=radar_var, command=toggle_radar, bootstyle="round-toggle").pack(side=tk.LEFT, padx=4)

    # Range Rings toggle
    rings_var = tk.BooleanVar(value=False)
    def toggle_rings():
        global map_static_dirty, rings_visible
        rings_visible = rings_var.get()
        map_static_dirty = True
        draw_map()
    ttk.Checkbutton(toolbar, text="Rings", variable=rings_var, command=toggle_rings, bootstyle="round-toggle").pack(side=tk.LEFT, padx=4)

    # Undo/Redo buttons
    ttk.Button(toolbar, text="Undo", command=perform_undo, bootstyle="secondary-outline").pack(side=tk.LEFT, padx=4)
    ttk.Button(toolbar, text="Redo", command=perform_redo, bootstyle="secondary-outline").pack(side=tk.LEFT)
    ...
    def fit_to_route():
        ...
        map_static_dirty = True
        draw_map()
    ttk.Button(toolbar, text="Fit", command=fit_to_route, bootstyle="info-outline").pack(side=tk.LEFT, padx=8)
    ttk.Label(toolbar, text="Left-Click: Measure • Middle-Drag: Pan", font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=8)

    # Create canvas
    map_canvas_global = tk.Canvas(map_window, bg="#222222", highlightthickness=0)
    map_canvas_global.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

    # Measure Tool Events
    def on_measure_start(event):
        global measure_start, measure_current
        wx, wy = map_canvas_to_world(event.x, event.y)
        measure_start = (wx, wy)
        measure_current = (wx, wy)
        draw_map()

    def on_measure_move(event):
        global measure_current
        if measure_start:
            wx, wy = map_canvas_to_world(event.x, event.y)
            measure_current = (wx, wy)
            draw_map()

    def on_measure_end(event):
        global measure_start, measure_current
        measure_start = None
        measure_current = None
        draw_map()

    map_canvas_global.bind('<Button-1>', on_measure_start)
    map_canvas_global.bind('<B1-Motion>', on_measure_move)
    map_canvas_global.bind('<ButtonRelease-1>', on_measure_end)

    # Context Menu
    context_menu = tk.Menu(map_window, tearoff=0)
    context_menu.add_command(label="Add Waypoint Here", command=lambda: add_manual_waypoint_direct(context_menu_pos[0], context_menu_pos[1]))
    context_menu.add_command(label="Fly To Here", command=lambda: fly_to_pos(context_menu_pos[0], context_menu_pos[1]))
    context_menu.add_separator()
    context_menu.add_command(label="Set Anchor Here", command=lambda: set_anchor_at(context_menu_pos[0], context_menu_pos[1]))

    context_menu_pos = (0, 0)

    def on_map_right_click(event):
        global context_menu_pos
        wx, wy = map_canvas_to_world(event.x, event.y)
        context_menu_pos = (wx, wy)
        context_menu.tk_popup(event.x_root, event.y_root)

    map_canvas_global.bind("<Button-3>", on_map_right_click)
...
def map_update_loop():
    """High-FPS loop for smooth radar animation"""
    global map_window
    ...
    if map_window is not None:
        map_window.after(16, map_update_loop)

def draw_map():
    """Draw the live tactical map with reduced flicker using layered redraws."""
    global map_canvas_global, map_view, map_static_dirty, _last_map_draw, radar_angle
    ...
    try:
        ...
        if map_view and all(k in map_view for k in ('min_x', 'max_x', 'min_y', 'max_y', 'width', 'height')):
            ...
            if not waypoints and not buoys:
                ...
                if ship_trail:
                    all_x.extend([p[0] for p in ship_trail])
                    all_y.extend([p[1] for p in ship_trail])
        ...
        if map_static_dirty:
            ...
            for x in range(start_x, end_x, grid_spacing):
                ...
                cx2, cy2 = world_to_canvas(x, max_y)
                map_canvas_global.create_line(cx1, cy1, cx2, cy2, fill='#333333', width=1, tags=('map_static',))
                map_canvas_global.create_text(cx1, height - 10, text=f"{x}", font=("Segoe UI", 8), fill='#666666', tags=('map_static',))
            ...
            for y in range(start_y, end_y, grid_spacing):
                ...
                map_canvas_global.create_line(cx1, cy1, cx2, cy2, fill='#333333', width=1, tags=('map_static',))
                map_canvas_global.create_text(20, cy1, text=f"{y}", font=("Segoe UI", 8), fill='#666666', tags=('map_static',))
            
            # Ship Trail (History)
            if len(ship_trail) > 1:
                trail_coords = []
                for p in ship_trail:
                    tx, ty = world_to_canvas(p[0], p[1])
                    trail_coords.extend([tx, ty])
                if len(trail_coords) >= 4:
                    map_canvas_global.create_line(trail_coords, fill='#555555', width=1, tags=('map_static',))
            ...
            if len(waypoints) > 1:
                for i in range(len(waypoints) - 1):
                    ...
                    map_canvas_global.create_line(x1, y1, x2, y2, fill='#00aaff', width=2, dash=(4, 4), tags=('map_static',))
            
            # Waypoints (polished look)
            for i, wp in enumerate(waypoints):
                cx, cy = world_to_canvas(wp['x'], wp['y'])
                
                # Outer ring
                map_canvas_global.create_oval(cx - 6, cy - 6, cx + 6, cy + 6,
                                              outline='#00aaff', width=2, tags=('map_static',))
                # Inner dot
                map_canvas_global.create_oval(cx - 2, cy - 2, cx + 2, cy + 2,
                                              fill='#00aaff', outline='', tags=('map_static',))
                
                # Label
                map_canvas_global.create_text(cx + 10, cy - 10, text=f"WP{i+1}",
                                              font=("Segoe UI", 9, 'bold'), fill='#00aaff', anchor='w', tags=('map_static',))
                
                # Waypoint heading indicator (subtle arrow)
                ...
                map_canvas_global.create_line(cx, cy, arrow_x, arrow_y, fill='#00aaff', width=1,
                                              arrow=tk.LAST, arrowshape=(8, 10, 3), tags=('map_static',))
            
            # MOB Marker
            if mob_active and mob_pos:
                mx, my = world_to_canvas(mob_pos['x'], mob_pos['y'])
                # Draw big red X
                map_canvas_global.create_line(mx-10, my-10, mx+10, my+10, fill='#ff0000', width=3, tags=('map_static',))
                map_canvas_global.create_line(mx-10, my+10, mx+10, my-10, fill='#ff0000', width=3, tags=('map_static',))
                map_canvas_global.create_text(mx, my-20, text="MOB", font=("Segoe UI", 12, "bold"), fill="#ff0000", tags=('map_static',))
            ...
            if buoys_visible:
                for i, buoy in enumerate(buoys):
                    ...
                    map_canvas_global.create_polygon(
                        ...
                        fill='#ffaa00', outline='#ff8800', width=1, tags=('map_static',)
                    )
                    # Add label
                    map_canvas_global.create_text(cx, cy - buoy_size - 8, text=f"B{i+1}",
                                                  font=("Segoe UI", 7), fill='#ff8800', tags=('map_static',))
            
            # Anchor Watch Circle
            if anchor_watch_active:
                ax, ay = world_to_canvas(anchor_pos['x'], anchor_pos['y'])
                r_px = anchor_radius * scale
                
                # Draw anchor icon
                map_canvas_global.create_text(ax, ay, text="⚓", font=("Segoe UI", 12), fill="#ffffff", tags=('map_static',))
                
                # Draw safe zone circle
                color = "#ff0000" if anchor_alarm else "#00ff00"
                map_canvas_global.create_oval(ax - r_px, ay - r_px, ax + r_px, ay + r_px,
                                            outline=color, width=2, dash=(5, 5), tags=('map_static',))

            # Range Rings
            if rings_visible:
                ship_cx, ship_cy = world_to_canvas(current_pos_x, current_pos_y)
                for r in [100, 200, 500, 1000]:
                    r_px = r * scale
                    map_canvas_global.create_oval(ship_cx - r_px, ship_cy - r_px, 
                                                ship_cx + r_px, ship_cy + r_px,
                                                outline='#444444', width=1, dash=(2, 4), tags=('map_static',))
                    map_canvas_global.create_text(ship_cx, ship_cy - r_px + 10, text=f"{r}m", 
                                                font=("Segoe UI", 7), fill='#666666', tags=('map_static',))

            # Title
            map_canvas_global.create_text(width / 2, 15, text='TACTICAL MAP',
                                          font=("Segoe UI", 10, 'bold'), fill='#888888', tags=('map_static',))

            map_static_dirty = False

        # Throttle dynamic layer redraws (High FPS: ~60 FPS = 0.016s)
        now = time.time()
        if now - _last_map_draw < 0.016:
            return
        _last_map_draw = now
        ...
        # --- Radar Visualization ---
        if radar_visible:
            # Rotate radar (faster speed for high FPS)
            radar_angle = (radar_angle + 4) % 360
            radar_len = max(width, height)
            
            # Draw radar sweep with trail
            for i in range(20):
                # Calculate trail angle (fading back)
                trail_angle = (radar_angle - i * 1.0) % 360
                trail_rad = math.radians(trail_angle)
                
                trail_end_x = ship_x + radar_len * math.sin(trail_rad)
                trail_end_y = ship_y - radar_len * math.cos(trail_rad)
                
                # Fade intensity
                if i == 0:
                    # Leading edge
                    map_canvas_global.create_line(ship_x, ship_y, trail_end_x, trail_end_y, 
                                                fill='#00ff00', width=2, tags=('map_dynamic',))
                else:
                    # Fading trail
                    intensity = int(60 * (1 - i/20))
                    color = f'#00{intensity:02x}00'
                    map_canvas_global.create_line(ship_x, ship_y, trail_end_x, trail_end_y, 
                                                fill=color, width=3, tags=('map_dynamic',))

            # Radar Noise (Simulated returns)
            for _ in range(3):
                if random.random() > 0.6:
                    dist = random.uniform(40, min(width, height)/2)
                    # Noise appears near the sweep line
                    noise_angle = radar_angle - random.uniform(0, 15)
                    noise_rad = math.radians(noise_angle)
                    
                    nx = ship_x + dist * math.sin(noise_rad)
                    ny = ship_y - dist * math.cos(noise_rad)
                    
                    # Draw blip
                    map_canvas_global.create_rectangle(nx-1, ny-1, nx+2, ny+2, 
                                                    fill='#00cc00', outline='', tags=('map_dynamic',))
            
            # Real Object Detection (Raycast with Persistence)
            objects_to_check = []
            if waypoints:
                objects_to_check.extend(waypoints)
            if buoys and buoys_visible:
                objects_to_check.extend(buoys)
            
            for obj in objects_to_check:
                dx = obj['x'] - current_pos_x
                dy = obj['y'] - current_pos_y
                dist = math.hypot(dx, dy)
                
                # Only show if within reasonable range
                if dist < 5000:
                    # Calculate bearing to object (0-360, 0=North)
                    obj_bearing = math.degrees(math.atan2(dx, dy)) % 360
                    
                    # Calculate angle difference for persistence
                    # How far has the radar passed this object?
                    # If radar_angle is 100 and obj_bearing is 90, diff is 10.
                    # If radar_angle is 10 and obj_bearing is 350, diff is 20.
                    angle_diff = (radar_angle - obj_bearing + 360) % 360
                    
                    # Persistence logic:
                    # If angle_diff is small (just hit), intensity is high.
                    # As angle_diff grows to 360, intensity fades.
                    
                    # Only draw if we've "scanned" it recently (or it's in the full circle memory)
                    # We map 0-360 degrees of age to opacity.
                    
                    # Brightness curve:
                    # 0-10 deg: Full brightness (Hit)
                    # 10-360 deg: Linear fade (Echo)
                    
                    if angle_diff < 15:
                        # Direct hit / active scan
                        intensity = 255
                        size = 6
                    else:
                        # Echo persistence
                        # Fade from 200 down to 0 over the full rotation
                        decay = (angle_diff - 15) / 345
                        intensity = int(200 * (1 - decay))
                        size = 4
                    
                    if intensity > 10:
                        color = f'#00{intensity:02x}00'
                        ox, oy = world_to_canvas(obj['x'], obj['y'])
                        map_canvas_global.create_oval(ox-size, oy-size, ox+size, oy+size, 
                                                    fill=color, outline=color, tags=('map_dynamic',))

        # Ship Glow/Halo
        map_canvas_global.create_oval(ship_x - 15, ship_y - 15, ship_x + 15, ship_y + 15, 
                                      fill='', outline='#ff4444', width=1, dash=(2, 4), tags=('map_dynamic',))
        ...
        # Calculate vertices
        nose_x = ship_x + ship_len * math.sin(ship_heading_rad)
        nose_y = ship_y - ship_len * math.cos(ship_heading_rad)
        
        rear_left_x = ship_x + ship_width * math.sin(ship_heading_rad - 2.5) # approx 140 deg
        rear_left_y = ship_y - ship_width * math.cos(ship_heading_rad - 2.5)
        
        rear_right_x = ship_x + ship_width * math.sin(ship_heading_rad + 2.5)
        rear_right_y = ship_y - ship_width * math.cos(ship_heading_rad + 2.5)
        
        # Draw filled ship
        map_canvas_global.create_polygon(nose_x, nose_y, rear_left_x, rear_left_y, rear_right_x, rear_right_y,
                                         fill='#ff4444', outline='white', width=1, tags=('map_dynamic',))
        ...
        if ship_speed > 0.5:
            ...
            map_canvas_global.create_line(ship_x, ship_y, vec_end_x, vec_end_y,
                                          fill='#ff00ff', width=2, arrow=tk.LAST, arrowshape=(8, 10, 4), tags=('map_dynamic',))
            map_canvas_global.create_text(vec_end_x + 10, vec_end_y, text=f"COG {calc_heading:.0f}°",
                                          font=("Segoe UI", 8), fill='#ff00ff', tags=('map_dynamic',))
        ...
        if 'autopilot_active' in globals() and autopilot_active and 'target_waypoint_index' in globals() and target_waypoint_index is not None and 0 <= target_waypoint_index < len(waypoints):
            ...
            map_canvas_global.create_line(ship_x, ship_y, target_x, target_y,
                                          fill='#00ff00', width=2, dash=(6, 4), arrow=tk.LAST, arrowshape=(10, 12, 5), tags=('map_dynamic',))
            ...
            map_canvas_global.create_text(mid_x, mid_y - 10, text=f"{distance:.0f} units",
                                          font=("Segoe UI", 9, 'bold'), fill='#00ff00', tags=('map_dynamic',))
            ...
            map_canvas_global.create_oval(target_x - buffer_radius, target_y - buffer_radius,
                                          ...
                                          outline='#00ff00', width=2, dash=(3, 2), tags=('map_dynamic',))

        # Measure Tool Line
        if measure_start and measure_current:
            sx, sy = world_to_canvas(measure_start[0], measure_start[1])
            ex, ey = world_to_canvas(measure_current[0], measure_current[1])
            
            # Draw line
            map_canvas_global.create_line(sx, sy, ex, ey, fill='#ffffff', width=2, dash=(2, 2), tags=('map_dynamic',))
            
            # Calculate stats
            dx = measure_current[0] - measure_start[0]
            dy = measure_current[1] - measure_start[1]
            dist = math.hypot(dx, dy)
            bearing = math.degrees(math.atan2(dx, dy)) % 360
            
            # Draw label at midpoint
            mx, my = (sx + ex) / 2, (sy + ey) / 2
            map_canvas_global.create_text(mx, my - 15, text=f"{dist:.1f} units\n{bearing:.0f}°", 
                                        font=("Segoe UI", 9, "bold"), fill="#ffffff", justify=tk.CENTER, tags=('map_dynamic',))
            
            # Draw start/end markers
            map_canvas_global.create_oval(sx-3, sy-3, sx+3, sy+3, fill='#ffffff', tags=('map_dynamic',))
            map_canvas_global.create_oval(ex-3, ey-3, ex+3, ey+3, fill='#ffffff', tags=('map_dynamic',))

        # Ship label
        map_canvas_global.create_text(ship_x, ship_y + 25, text='SHIP',
                                      font=("Segoe UI", 10, 'bold'), fill='#ff4444', tags=('map_dynamic',))
        ...
        map_canvas_global.create_line(scale_x, scale_y, scale_x + scale_pixels, scale_y,
                                      fill='#888888', width=2, tags=('map_dynamic',))
        map_canvas_global.create_text(scale_x + scale_pixels / 2, scale_y - 10,
                                      text=f"{scale_length} units", font=("Segoe UI", 8), fill='#888888', tags=('map_dynamic',))
...
def on_minimize(event):
    """Handle window minimize event - hide to tray"""
    # Check if the event is for the root window and state is iconic
...
def setup_global_hotkeys():
    try:
        ...
        def _a_up(e):
            toggle_position_lock_hotkey()  # A - lock position (X & Y)
        def _shift_up(e):
        ...
        keyboard.on_release_key('a', _a_up, suppress=False)
...
root.geometry("350x600")  # Increased size for better layout
...
status_label = ttk.Label(status_frame, text="Ready", bootstyle="inverse-secondary")
status_label.pack(fill=tk.X)

# Current position display
info_frame = ttk.Frame(root, padding="3")
info_frame.pack(fill=tk.X)
read_label = ttk.Label(info_frame, text="Current: X=--, Y=--, H_raw=--", font=("Segoe UI", 9))
read_label.pack()
speed_label = ttk.Label(info_frame, text="Speed: 0.0 u/s | COG: 0.0°", font=("Segoe UI", 9, "bold"), bootstyle="info")
speed_label.pack()
nav_info_frame = ttk.Frame(info_frame)
nav_info_frame.pack()
eta_label = ttk.Label(nav_info_frame, text="ETA: --:--", font=("Segoe UI", 10, "bold"), bootstyle="warning")
eta_label.pack(side=tk.LEFT, padx=5)
xte_label = ttk.Label(nav_info_frame, text="XTE: --", font=("Segoe UI", 10, "bold"), bootstyle="danger")
xte_label.pack(side=tk.LEFT, padx=5)
drift_label = ttk.Label(nav_info_frame, text="Drift: 0.0°", font=("Segoe UI", 10, "bold"), bootstyle="info")
drift_label.pack(side=tk.LEFT, padx=5)

# Retry connect button if not connected
retry_btn = ttk.Button(info_frame, text="Retry Connect", command=lambda: retry_connect(), bootstyle="warning-outline")
retry_btn.pack(pady=1)

topmost_btn = ttk.Button(info_frame, text="Always On Top: ON", command=toggle_always_on_top, bootstyle="info-outline")
...
x_lock_label = ttk.Label(x_lock_frame, text="Unlocked", bootstyle="success", 
                         font=("Arial", 8, "bold"))
x_lock_label.pack(side=tk.LEFT, padx=2)

# Anchor Watch Button
def toggle_anchor_watch():
    global anchor_watch_active, anchor_pos, anchor_alarm
    if not anchor_watch_active:
        # Enable
        x, y, _ = read_position()
        if x is not None:
            anchor_pos = {'x': x, 'y': y}
        ...
        anchor_watch_active = False
        anchor_alarm = False
        anchor_btn.config(text="⚓ Set Anchor", bootstyle="secondary-outline")
        status_label.config(text="Anchor watch disabled")
    draw_map()

anchor_btn = ttk.Button(left_frame, text="⚓ Set Anchor", command=toggle_anchor_watch, bootstyle="secondary-outline", width=12)
anchor_btn.pack(pady=2)
...
y_lock_label = ttk.Label(y_lock_frame, text="Unlocked", bootstyle="success", 
                         font=("Arial", 8, "bold"))
...
heading_lock_label = ttk.Label(lock_frame, text="Unlocked", bootstyle="success", 
                               font=("Arial", 8, "bold"))
heading_lock_label.pack(side=tk.LEFT, padx=2)

canvas = tk.Canvas(right_frame, width=120, height=120, bg="#2b2b2b", highlightthickness=0)
...
ttk.Button(waypoint_control_frame, text="Clear All", command=clear_waypoints, bootstyle="danger-outline").pack(side=tk.LEFT, padx=1)
ttk.Button(waypoint_control_frame, text="Save", command=save_waypoints, bootstyle="secondary-outline").pack(side=tk.LEFT, padx=1)
ttk.Button(waypoint_control_frame, text="Load", command=load_waypoints, bootstyle="secondary-outline").pack(side=tk.LEFT, padx=1)
ttk.Button(waypoint_control_frame, text="📍 Map", command=open_map_window, bootstyle="info").pack(side=tk.LEFT, padx=5)
autopilot_btn = ttk.Button(waypoint_control_frame, text="▶️ Autopilot", command=toggle_autopilot, bootstyle="success")
autopilot_btn.pack(side=tk.LEFT, padx=5)

# MOB Button
def toggle_mob():
    global mob_active, mob_pos, waypoints, autopilot_active, target_waypoint_index
    ...
    if not mob_active:
        ...
        waypoints.insert(0, mob_wp)
        target_waypoint_index = 0
        autopilot_active = True
        
        status_label.config(text="!!! MAN OVERBOARD - RETURNING !!!", bootstyle="danger")
        mob_btn.config(text="CANCEL MOB", bootstyle="danger")
        
        # Force map redraw to show icon
        globals()['map_static_dirty'] = True
        draw_map()
        ...
        mob_active = False
        mob_pos = None
        mob_btn.config(text="🆘 MOB", bootstyle="danger-outline")
        status_label.config(text="MOB Cancelled")
        globals()['map_static_dirty'] = True
        draw_map()

mob_btn = ttk.Button(waypoint_control_frame, text="🆘 MOB", command=toggle_mob, bootstyle="danger-outline")
mob_btn.pack(side=tk.LEFT, padx=5)
...
# Waypoint list display (Tabbed Interface)
tab_control = ttk.Notebook(root)
tab_control.pack(fill=tk.BOTH, expand=True, padx=3, pady=1)

# Tab 1: Waypoints
waypoint_tab = ttk.Frame(tab_control)
tab_control.add(waypoint_tab, text='Route')

# Create Treeview for waypoints
columns = ('#', 'time', 'x', 'y', 'heading')
waypoint_tree = ttk.Treeview(waypoint_tab, columns=columns, show='headings', height=5)
...
tab_control.add(log_tab, text='Event Log')

log_text = scrolledtext.ScrolledText(log_tab, height=5, state='disabled', font=("Consolas", 9))
...
def generate_search_pattern():
    ...
    try:
        ...
        dialog.title("Search Pattern")
        dialog.geometry("300x200")
        
        ttk.Label(dialog, text="Leg Length (units):").pack(pady=5)
        leg_entry = ttk.Entry(dialog)
        leg_entry.pack(pady=5)
        leg_entry.insert(0, "500")
        
        ttk.Label(dialog, text="Number of Turns:").pack(pady=5)
        turns_entry = ttk.Entry(dialog)
        turns_entry.pack(pady=5)
        turns_entry.insert(0, "10")
        
        def create_pattern():
            try:
                leg = float(leg_entry.get())
                turns = int(turns_entry.get())
                
                start_x, start_y = current_pos_x, current_pos_y
                if waypoints:
                    start_x, start_y = waypoints[-1]['x'], waypoints[-1]['y']
                
                # Expanding square logic
                # N, E, S, W directions
                ...
                for i in range(turns):
                    ...
                    cx += dx * current_leg_len
                    cy += dy * current_leg_len
                    
                    add_manual_waypoint_direct(cx, cy)
                    
                    current_dir_idx = (current_dir_idx + 1) % 4
                    if i % 2 == 1: # Increase leg length every 2 turns
                        current_leg_len += leg
                
                dialog.destroy()
                status_label.config(text="Search pattern generated")
                
            except ValueError:
                messagebox.showerror("Error", "Invalid input")

        ttk.Button(dialog, text="Generate", command=create_pattern, bootstyle="success").pack(pady=10)
        
    except Exception as e:
        print(f"Pattern gen error: {e}")

def add_manual_waypoint_direct(x, y):
    """Helper to add waypoint without UI reading"""
    global waypoints
    wp = {
        'time': 0,
        'x': x,
        'y': y,
        'heading_raw': 0, # Default North
        'original_index': len(waypoints)
    }
    waypoints.append(wp)
    update_waypoint_list()
    globals()['map_static_dirty'] = True
...
# Smoothing controls
smooth_frame = ttk.Labelframe(root, text="Path Tools", padding="3")
...
ttk.Button(smooth_btn_frame, text="Search Pattern", command=generate_search_pattern, bootstyle="info-outline").pack(side=tk.LEFT, padx=1)
...
replay_btn = ttk.Button(replay_frame, text="▶ Start", command=start_replay, bootstyle="success")
replay_btn.pack(side=tk.LEFT, padx=1)

stop_btn = ttk.Button(replay_frame, text="⏹ Stop", command=stop_replay, state='disabled', bootstyle="danger")
...
root.bind_all('<KeyRelease-a>', lambda e: toggle_position_lock_hotkey())  # A - lock position (X & Y)
